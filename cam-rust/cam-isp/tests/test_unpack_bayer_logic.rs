...
fn test_unpack_single_value() {
    ...
    assert!(
        (a_val - expected_a).abs() < 1e-3,
        "A: got {} expected {}",
        a_val,
        expected_a
    );
    assert!(
        (b_val - expected_b).abs() < 1e-3,
        "B: got {} expected {}",
        b_val,
        expected_b
    );
...
fn test_unpack_format_conversion() {
    ...
    let expected_a = vec![vec![100.0, 300.0], vec![500.0, 700.0]];
    let expected_b = vec![vec![200.0, 400.0], vec![600.0, 800.0]];
    ...
    for i in 0..h * w {
        ...
        assert!(
            (a.to_f32() - expected_a_val).abs() < 1e-3,
            "A[{},{}] got {} expected {}",
            row,
            col,
            a.to_f32(),
        ...
        assert!(
            (b.to_f32() - expected_b_val).abs() < 1e-3,
            "B[{},{}] got {} expected {}",
            row,
            col,
            b.to_f32(),
