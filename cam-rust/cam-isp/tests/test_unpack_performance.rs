...
fn test_unpack_performance_fhd_4k() {
    ...
    for res in &RESOLUTIONS {
        ...
        println!("FP16 unpack: {:.2?} ({:.2} MB output)", fp16_time, fp16_output_size_mb);
        println!("FP32 unpack: {:.2?} ({:.2} MB output)", fp32_time, fp32_output_size_mb);
        ...
        for i in 0..total_pixels.min(100) {
            ...
            assert!((fp16_a - fp32_a).abs() < 1e-3, 
                "FP16 vs FP32 mismatch at pixel {} A: {} vs {}", i, fp16_a, fp32_a);
            assert!((fp16_b - fp32_b).abs() < 1e-3,
                "FP16 vs FP32 mismatch at pixel {} B: {} vs {}", i, fp16_b, fp32_b);
        }
    }
}
...
fn test_onnx_model_building_performance() {
    ...
    for res in &RESOLUTIONS {
        ...
        println!("Model size: {} bytes ({:.2} KB)", 
            onnx_bytes.len(), onnx_bytes.len() as f64 / 1024.0);
        println!();
    }
}
...
fn test_fp16_fp32_memory_usage() {
    ...
    for res in &RESOLUTIONS {
        ...
        println!("Output (FP16): {:.2} MB ({:.1}% of input)", 
            fp16_size_mb, fp16_size_mb / input_size_mb * 100.0);
        println!("Output (FP32): {:.2} MB ({:.1}% of input)", 
            fp32_size_mb, fp32_size_mb / input_size_mb * 100.0);
        println!("FP16 vs FP32 savings: {:.1}%", 
...
fn test_prepare_fp16_fp32_buffers() {
    ...
    for res in &RESOLUTIONS {
        ...
        println!("FP16 buffer: {} elements, {:.2?} to prepare", 
            fp16_buffer.len(), fp16_time);
        println!("FP32 buffer: {} elements, {:.2?} to prepare", 
            fp32_buffer.len(), fp32_time);
        
        // Verify first few values
        for i in 0..5 {
            let fp16_val = fp16_buffer[i].to_f32();
            let fp32_val = fp32_buffer[i];
            println!("  Value {}: FP16={:.6}, FP32={:.6}", 
                i, fp16_val, fp32_val);
        }
...
fn test_performance_logging() {
    ...
    for m in &metrics {
        println!("{:<8} {:<12.2} {:<12.2} {:<12.2} {:<12.2} {:<12.2} {:<12.2}",
            m.resolution,
    ...
    for m in &metrics {
        ...
        println!("{} FP16 vs FP32: {:.2}x faster", m.resolution, fp16_fp32_ratio);
    }
}
