...
use cam_isp::mnn_sys::{mnn_run_true_zero_copy, MnnBackendType, MnnInterpreterSafe};
...
fn test_cast(to_type: i64, elem_type: i32, label: &str) {
    ...
    let sess = interp
        .create_session(MnnBackendType::Cpu, 1)
        .expect("session");
    ...
    let out_bytes =
        unsafe { std::slice::from_raw_parts(out_f32.as_ptr() as *const u8, n as usize * 4) };

    print!("Bytes[0..16]: ");
    for b in out_bytes.iter().take(16) {
        print!(" {:02x}", b);
    }
    println!();
    ...
    if let Some(t) = interp.get_first_output(&sess) {
        ...
        if dt == 5 {
            ...
            println!("→ FLOAT32 (Cast removed by MNN) ❌");
        } else {
            println!("→ Unknown dtype={}", dt);
        }
    }
}
