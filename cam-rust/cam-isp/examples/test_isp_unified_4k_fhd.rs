...
fn main() {
    ...
    let mut engine = match cam_isp::engine::select_engine_by_name("mnn_vulkan") {
        Some(e) => e,
        None => {
            eprintln!("Failed to select Vulkan engine: {}, falling back", "no engine");
    ...
    for (input_name, input_w, input_h) in input_dimensions {
        for (output_name, output_w, output_h) in output_dimensions {
            println!("
=== Testing {} {} → {} {} with {} profile ===", input_name, input_w, output_name, output_h, profile.label);
            ...
            match engine.process(&params) {
                Ok(frame) => {
                    ...
                    println!("✅ Success: {}ms ({} FPS)", elapsed_ms, 1000 / elapsed_ms.max(1));
                    println!("   Raw input:  {}×{} (Bayer)", input_w, input_h);
                    println!("   Output:     {}×{} {:?} ({} bytes)", frame.width, frame.height, frame.format, frame.data.len());
                    ...
                    if frame.data.len() == expected_bytes as usize {
                        println!("✅ Size correct: {} = {}×{}×4", frame.data.len(), output_w, output_h);
                        ...
                        println!("❌ Size mismatch: got {} bytes, expected {} bytes ({}×{})", 
                                frame.data.len(), expected_bytes, output_w, output_h);
                    }
                }
                Err(err) => {
                    println!("❌ Error: {:?}", err);
