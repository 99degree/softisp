...
fn main() {
    ...
    for n in 1..=blocks.len() {
        let mut subset: Vec<Box<dyn IspBlock>> = blocks[..n].iter()
            .map(|b| {
                // Rebuild each block by its id
                match b.id() {
                    "raw_input" => Box::new(cam_isp::blocks::RawInputBlock::new().with_elem_type(6).with_concrete_width(960)) as Box<dyn IspBlock>,
                    "unpack_cfa" => Box::new(cam_isp::blocks::UnpackCfaBlock::new().with_concrete_width(1920).with_blc(true)),
                    ...
                    _ => { println!("  UNKNOWN block: {}", b.id()); Box::new(cam_isp::blocks::IdentityBlock::new("unknown")) }
                }
            })
            .collect();
