...
fn main() {
    ...
    println!(
        "Engine built OK (backend={}, native={})",
        engine_name, use_native
    );
    println!();
    ...
    match &result {
        Ok(frame) => println!(
            "Sequential 1 frame: {:6.1}ms → {}B output",
    ...
    println!(
        "Parallel {} frames:  {:6.1}ms wall (theoretical sequential would be ~{:.1}ms)",
        n_parallel,
        par_elapsed.as_secs_f64() * 1000.0,
        seq_elapsed.as_secs_f64() * 1000.0 * n_parallel as f64
    );
    println!(
        "Speedup: {:.1}× (ideal would be {:.1}× with {} sessions)",
