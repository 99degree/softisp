...
fn bench_heavy(name: &str, w: u32, h: u32, fmt: OutputFormat, iters: u32) {
    ...
    for v in &raw { raw_bytes.extend_from_slice(&v.to_le_bytes()); }
    ...
    for _ in 0..iters {
        ...
        match engine.process(&params) {
            Ok(frame) => {
                ...
                if times.len() == 1 {
                    println!("  First frame: {}×{} data={}B", frame.width, frame.height, frame.data.len());
                }
            }
            Err(e) => {
                errors += 1;
                if errors == 1 { println!("  ERROR: {:?}", e); }
            }
        }
    }
    times.sort_by(|a, b| a.partial_cmp(b).unwrap());
    if times.is_empty() {
        println!("  {:6} {:12}  {} blocks  ALL {} FRAMES FAILED", name, format!("{:?}", fmt), block_count, errors);
        return;
    }
    let avg: f64 = times.iter().sum::<f64>() / times.len() as f64;
    let p50 = times[times.len() / 2];
    println!("  {:6} {:12}  {} blocks  {:>6.2}ms ({:>4.0} FPS)  p50={:.2}ms  errors={}",
        name, format!("{:?}", fmt), block_count, avg, 1000.0 / avg, p50, errors);
}
