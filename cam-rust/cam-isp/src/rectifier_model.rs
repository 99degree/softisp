...
pub fn generate_rectifier_model() -> Vec<u8> {
    ...
    // Input/Output specs
    let inputs = vec![
        Proto::value_info("input", &[Proto::tensor_dim_value(1), Proto::tensor_dim_value(267)], 1)
    ];
    
    let outputs = vec![
        Proto::value_info("output", &[Proto::tensor_dim_value(1), Proto::tensor_dim_value(20)], 1)
    ];
