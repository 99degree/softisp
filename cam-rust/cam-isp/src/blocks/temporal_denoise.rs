...
impl IspBlock for TemporalDenoiseBlock {
    ...
    fn nodes(&self) -> Vec<Vec<u8>> {
        ...
        nodes.push(Proto::node(
            "Sub",
            &[&self.input_source, &self.prev_frame_source],
            &[&diff],
            &[],
        ));
        ...
        nodes.push(Proto::node("Sub", &[&one_name, &mask], &[&inv_mask], &[]));
        ...
        nodes.push(Proto::node(
            "Mul",
            &[&self.prev_frame_source, &inv_mask],
            &[&prev_weighted],
            &[],
        ));
        nodes.push(Proto::node(
            "Mul",
            &[&prev_weighted, &blend_w],
            &[&prev_weighted],
            &[],
        ));
        ...
        nodes.push(Proto::node(
            "Mul",
            &[&self.input_source, &mask],
            &[&curr_weighted],
            &[],
        ));
        nodes.push(Proto::node(
            "Mul",
            &[&curr_weighted, &curr_weight],
            &[&curr_weighted],
            &[],
        ));
...
mod tests {
    ...
    fn test_temporal_denoise_has_two_inputs() {
        let block = TemporalDenoiseBlock::new().with_threshold(0.05);
