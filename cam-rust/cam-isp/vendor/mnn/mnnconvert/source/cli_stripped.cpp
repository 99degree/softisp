//
//  cli_stripped.cpp
//  mnnconvert - ONNX-only minimal converter
//
//  Stripped down from tools/converter/source/common/cli.cpp
//  Only supports ONNX -> MNN conversion (plus MNN/JSON auxiliary ops)
//
...
static std::string _getDataType(const halide_type_t& type) {
    switch (type.code) {
        case halide_type_float:
            if (type.bits == 32) return "float";
            if (type.bits == 16) return "half";
            break;
        case halide_type_uint:
            if (type.bits == 32) return "uint32";
            if (type.bits == 16) return "uint16";
            if (type.bits == 8) return "uint8";
            break;
        case halide_type_int:
            if (type.bits == 32) return "int32";
            if (type.bits == 16) return "int16";
            if (type.bits == 8) return "int8";
            break;
        default: break;
    }
    return "Unknown";
...
static std::string _getFormatString(MNN::Express::Dimensionformat format) {
    switch (format) {
        case MNN::Express::NCHW:  return "NCHW";
        case MNN::Express::NHWC:  return "NHWC";
        case MNN::Express::NC4HW4: return "NC4HW4";
        default: break;
    }
    return "Unknown";
}
...
bool Cli::initializeMNNConvertArgs(modelConfig& modelPath, int argc, char** argv) {
    ...
    options.allow_unrecognised_options().add_options()(
        ...
        "modelFile", "ONNX model file, ex: *.onnx",
        cxxopts::value<std::string>())(
        "batch", "if model input's batch is not set, set as the batch size you set",
        cxxopts::value<int>())(
        "keepInputFormat", "keep input dimension format or not, default: true",
        cxxopts::value<bool>())(
        "optimizeLevel",
        "graph optimize option, 0: don't run optimize, 1: use graph optimize only for "
        "every input case is right, 2: normally right but some case may be wrong, default 1",
        cxxopts::value<int>())(
        "optimizePrefer", "graph optimize option, 0 for normal, 1 for smallest, 2 for fastest",
        cxxopts::value<int>())(
        "MNNModel", "output MNN model, ex: *.mnn",
        cxxopts::value<std::string>())(
        "fp16", "save Conv's weight/bias in half_float data type")(
        "benchmarkModel",
        "Do NOT save big size data, such as Conv's weight, BN's gamma, beta, mean and variance etc. "
        "Only used to test the cost of the model")(
        "bizCode", "MNN Model Flag, ex: MNN",
        cxxopts::value<std::string>())(
        "debug", "Enable debugging mode.")(
        "forTraining", "whether or not to save training ops BN and Dropout, default: false",
        cxxopts::value<bool>())(
        "weightQuantBits",
        "save conv/matmul/LSTM float weights to int8 type, only optimize for model size, "
        "2-8 bits, default: 0 (no weight quant)",
        cxxopts::value<int>())(
        "weightQuantAsymmetric",
        "use asymmetric quant method for weight quant (default: false)",
        cxxopts::value<bool>())(
        "weightQuantBlock",
        "block-wise weight quant block size, default: -1 (channel-wise)",
        cxxopts::value<int>())(
        "hqq",
        "use HQQ quant method to improve accuracy, default: false",
        cxxopts::value<bool>())(
        "weightQuantScaleBit",
        "bit-width for quant scale/zero-point, 32=fp32 (default), 16=fp16",
        cxxopts::value<int>())(
        "compressionParamsFile",
        "path to compression parameters file",
        cxxopts::value<std::string>())(
        "OP", "print supported ops",
        cxxopts::value<bool>())(
        "saveStaticModel", "save static model with fix shape, default: false",
        cxxopts::value<bool>())(
        "targetVersion", "target MNN engine version",
        cxxopts::value<float>())(
        "customOpLibs", "custom op libs ex: libmy_add.so;libmy_sub.so",
        cxxopts::value<std::string>())(
        "info", "dump MNN model info")(
        "authCode", "code for model authentication",
        cxxopts::value<std::string>())(
        "inputConfigFile", "input config file for static model",
        cxxopts::value<std::string>())(
        "testdir", "test directory for verify conversion",
        cxxopts::value<std::string>())(
        "testconfig", "test config json",
        cxxopts::value<std::string>())(
        "thredhold", "max error rate for test, default: 0.01",
        cxxopts::value<float>())(
        "JsonFile", "dump MNN model to JsonFile",
        cxxopts::value<std::string>())(
        "alignDenormalizedValue",
        "align denormalized float as zero, default: 1",
        cxxopts::value<int>())(
        "detectSparseSpeedUp",
        "detect weight sparsity for sparse speedup")(
        "saveExternalData", "save weight to external bin file",
        cxxopts::value<bool>())(
        "useGeluApproximation", "Use Gelu Approximation Instead of ERF",
        cxxopts::value<int>())(
        "convertMatmulToConv", "convert matmul with constant input to conv, default: 1",
        cxxopts::value<int>())(
        "transformerFuse", "fuse key transformer op, like attention, default: false",
        cxxopts::value<bool>())(
        "groupConvNative", "keep native group convolution, default: false",
        cxxopts::value<bool>())(
        "allowCustomOp", "allow custom op when convert, default: false",
        cxxopts::value<bool>())(
        "useOriginRNNImpl",
        "don't use While Module for LSTM/GRU, use origin op",
        cxxopts::value<bool>())(
        "splitBlockQuant", "Split Block Quant Convolution")(
        "dumpPass",
        "Enable verbose output for each optimization pass");
    ...
    if (result.count("framework")) {
        ...
        if ("ONNX" == frameWork) {
            ...
            std::cout << "mnnconvert only supports -f ONNX (or MNN/JSON for auxiliary operations)." << std::endl;
            return false;
        }
    } else {
        std::cout << options.help({""}) << std::endl;
        DLOG(INFO) << "framework required, use -f ONNX";
        return false;
    }
    ...
    if (result.count("OP")) {
        ...
        if (res == OpCount::get()->getMap().end()) return false;
    ...
    if (result.count("modelFile")) {
        ...
        DLOG(INFO) << "modelFile not set, use --modelFile to set!";
        return false;
    }
    ...
    if (result.count("MNNModel")) {
        modelPath.MNNModel = result["MNNModel"].as<std::string>();
    } else if (result.count("JsonFile")) {
        modelPath.MNNModel = result["JsonFile"].as<std::string>();
        modelPath.mnn2json = true;
    } else if (result.count("info") && modelPath.model == modelConfig::MNN) {
        modelPath.dumpInfo = true;
        return true;
    } else {
        DLOG(INFO) << "MNNModel output path not set, use --MNNModel to set!";
        return false;
    }

    if (result.count("targetVersion")) {
        modelPath.targetVersion = result["targetVersion"].as<float>();
        std::cout << "TargetVersion is " << modelPath.targetVersion << std::endl;
    }

    if (result.count("bizCode"))
        modelPath.bizCode = result["bizCode"].as<std::string>();
    else {
        MNN_ERROR("Don't have bizCode, use MNNTest for default\n");
        modelPath.bizCode = "MNNTest";
    }

    if (result.count("inputConfigFile"))
        modelPath.inputConfigFile = result["inputConfigFile"].as<std::string>();

    if (result.count("fp16")) modelPath.saveHalfFloat = true;
    ...
    if (result.count("hqq")) {
        if (modelPath.weightQuantAsymmetric)
            ...
            std::cout << "Warning: HQQ requires weightQuantAsymmetric. Disabling HQQ." << std::endl;
    }
    if (result.count("forTraining")) modelPath.forTraining = true;
    if (result.count("batch")) modelPath.defaultBatchSize = result["batch"].as<int>();
    ...
    if (result.count("weightQuantBits")) {
        ...
        if (modelPath.useHQQ) MNN_PRINT("Use HQQ to quant weight\n");
    }
    ...
    if (result.count("saveStaticModel")) modelPath.saveStaticModel = true;
    if (result.count("optimizePrefer"))
        modelPath.optimizePrefer = result["optimizePrefer"].as<int>();
    if (result.count("compressionParamsFile"))
        modelPath.compressionParamsFile = result["compressionParamsFile"].as<std::string>();
    ...
    if (result.count("splitBlockQuant")) modelPath.splitQuantBlock = true;
    if (result.count("alignDenormalizedValue"))
        modelPath.alignDenormalizedValue = result["alignDenormalizedValue"].as<int>();
    if (result.count("detectSparseSpeedUp")) modelPath.detectSparseSpeedUp = true;
    ...
    if (result.count("saveExternalData")) modelPath.saveExternalData = true;
    if (result.count("transformerFuse")) modelPath.transformerFuse = true;
    if (result.count("groupConvNative")) modelPath.groupConvNative = true;
    if (result.count("allowCustomOp")) modelPath.allowCustomOp = true;
    if (result.count("useOriginRNNImpl")) modelPath.useOriginRNNImpl = true;
    if (result.count("dumpPass")) modelPath.dumpPass = true;
...
static void _reorderInputs(const std::vector<std::string>& inputNames, MNN::NetT* netT) {
    if (!inputNames.empty()) {
        ...
        for (auto& op : oplists) {
            if (nullptr == op.get()) continue;
            if (op->type != MNN::OpType_Input || op->outputIndexes.empty()) continue;
        ...
        for (auto& op : oplists) {
            if (nullptr != op.get()) netT->oplists.emplace_back(std::move(op));
        }
    }
}
...
bool Cli::convertModel(modelConfig& modelPath) {
    ...
    std::cout << "Start to Convert ONNX Model To MNN Model..., target version: " << modelPath.targetVersion << std::endl;
    ...
    if (modelPath.defaultBatchSize > 0) {
        for (const auto& op : netT->oplists) {
            if (op->type != OpType_Input || nullptr == op->main.AsInput()) continue;
    ...
    if (1 == modelPath.optimizeLevel && modelPath.model == modelConfig::MNN)
        expectedPass = {"TranslateJsonOp", "FuseDupOp", "RemoveInvalidCast"};
    ...
    if (needOptimize) {
        ...
        int error = writeFb(newNet, modelPath, std::move(metaOp));
        if (0 == error)
        ...
        int error = writeFb(netT, modelPath, std::move(metaOp));
        if (0 == error)
            std::cout << "Converted Success!" << std::endl;
        else
            std::cout << "Converted Failed!" << std::endl;
    ...
    if (modelPath.testDir.size() > 0) {
        std::cout << "Check convert result by " << modelPath.testDir
                  << ", threshold is " << modelPath.testThredhold << std::endl;
        Cli::testconvert(modelPath.MNNModel, modelPath.testDir, modelPath.testThredhold, modelPath.testConfig);
    }
    return true;
}

static bool compareOutput(MNN::Express::VARP output, const std::string& directName,
                          const std::string& name, MNN::Express::Dimensionformat dataFormat,
                          int order, float maxError) {
    if (output == nullptr) {
    ...
    for (int i = 0; i < info->size; ++i) {
        if (fabs(outputPtr[i]) == INFINITY || (outputPtr[i]) != outputPtr[i]) {
...
int Cli::testconvert(const std::string& defaultCacheFile, const std::string& directName,
                     float maxErrorRate, const std::string& backendConfigJson) {
        ...
        if (document.HasMember("inputs")) {
            ...
            for (auto iter = inputsInfo.begin(); iter != inputsInfo.end(); iter++) {
                ...
                if (obj.HasMember("shape")) {
                    ...
                    for (auto iter2 = dims.begin(); iter2 != dims.end(); iter2++)
                        shapes.emplace_back(iter2->GetInt());
    ...
    if (!backendConfigJson.empty()) {
        do {
            ...
            if (configDoc.HasParseError()) break;
    ...
    std::shared_ptr<MNN::Express::Module> net(
        MNN::Express::Module::load(inputNames, outputNames, defaultCacheFile.c_str(), rtmgr, &mConfig));
...
#define LOAD_DATA(TYPE)                                                                    \
    if (inputInfo.find(inputName) != inputInfo.end()) {                                    \
        ...
        for (int i = 0; i < info->size; ++i) ptr[i] = value;                               \
    } else {                                                                               \
        ...
        for (int i = 0; i < info->size; ++i) { double tempValue; inputOs >> tempValue; ptr[i] = tempValue; } \
    }
    for (int i = 0; i < inputs.size(); ++i) {
        auto inputName = inputNames[i];
        auto shapeIter = inputShape.find(inputName);
        if (shapeIter != inputShape.end())
            inputs[i] = _Input(shapeIter->second, mInfo->defaultFormat, mInfo->inputs[i].type);
        else
...
#undef LOAD_DATA
    ...
    for (int i = 0; i < compareSize; ++i) {
        bool success = compareOutput(outputs[i], directName, outputNames[i], mInfo->defaultFormat, i, maxErrorRate);
        if (!success) {
            modelError = true;
    ...
    if (modelError) {
        MNN_PRINT("Save mnn result to .error directory\n");
        for (int i = 0; i < compareSize; ++i) {
            auto v = outputs[i];
            auto name = outputNames[i];
            auto info = v->getInfo();
...
bool Cli::mnn2json(const char* modelFile, const char* jsonFile, int flag) {
    ...
    if (flag > 3) {
        MNN_PRINT("Don't add convweight\n");
        ...
        auto treatFunction = [&](MNN::OpT* opParam) {
            ...
            if (type == MNN::OpParameter::OpParameter_Convolution2D) {
                ...
                if (param->weightH) param->weightH->float32s.clear();
                if (param->weightI) param->weightI->float32s.clear();
                if (param->bias) param->bias->float32s.clear();
            }
        };
