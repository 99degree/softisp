...
int main() {
    ...
    // const at binding 0, using INT32 data type (raw bytes matching shader layout)
    {
        std::unique_ptr<MNN::AttributeT> a(new MNN::AttributeT);
        a->key = "const";
        a->i = 0;
        a->tensor.reset(new MNN::BlobT);
        a->tensor->dataType = MNN::DataType_DT_INT32;
        // Store as int32_t - for the uints, use proper integer values
        // For floats, store the IEEE 754 bit pattern
        ...
        // Store floats as int32 bit patterns
